...
int main(void)
{
	...
	for (;; status = 0, meth_code = 0, req_id = INT_MAX)
	{
		...
		{
			status = 404, sender_closer_adv(in, ip, meth, path);
			continue;
		}
		status = meth_code == 1 ? header_parse(body)
			: path_verification(path, "/todos");
		sender_closer_adv(in, ip, meth, path);
	}
	return (EXIT_SUCCESS);
}
...
{
	int valid = 0;
	char base[PATH_MAX], quer[128];

	if (sscanf(path, "%[^?]?%s", base, quer) > 0 && !strcmp(base, expected))
		valid = 1;
